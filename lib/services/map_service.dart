...
/// Self-hosted routing with Uzbekistan map data
class OSRMService {
  // Self-hosted OSRM server (Uzbekistan map, MLD algorithm)
  // Accessible via API proxy: api.deliverymaker.uz/v1/logistics/routing/*
  // Direct access for mobile: through API backend
  static const String _baseUrl = 'https://api.deliverymaker.uz/osrm';

  // Fallback: public OSRM demo (rate-limited, NOT for production)
  static const String _demoUrl = 'https://router.project-osrm.org';

  // Production self-hosted OSRM
  static const String _selfHostedUrl = 'https://api.deliverymaker.uz/osrm';
  ...
  static Future<RouteResult> getRoute(
    ...
    final url = Uri.parse(
      '$_selfHostedUrl/route/v1/$profile/$coords'
      '?overview=full'
      '&geometries=geojson'
      '&steps=true'
      '&annotations=true'
  ...
  static Future<List<LatLng>> optimizeWaypoints(
    ...
    final url = Uri.parse(
      '$_selfHostedUrl/trip/v1/$profile/$coords'
      '?source=first'
      '&destination=last'
      '&roundtrip=false'
    );
