...
class CustomerRepository {
  ...
  Future<List<CustomerEntity>> searchCustomers(String query) async {
    ...
    if (query.isEmpty) {
      return await db.collection<CustomerEntity>()
          .where()
          .sortByCreatedAtDesc()
          .limit(50)
          .findAll();
    ...
    // Search by phone
    final byPhone = await db.collection<CustomerEntity>()
        .filter()
        .phoneContains(query)
        .findAll();

    // Search by name
    final byName = await db.collection<CustomerEntity>()
        .filter()
  ...
  Future<CustomerEntity?> getCustomerById(String customerId) async {
    ...
    if (int.tryParse(customerId) != null) {
      final byLocalId = await db.collection<CustomerEntity>().get(int.parse(customerId));
      if (byLocalId != null) return byLocalId;
    }

    // Try by server ID
    final byServerId =
        await db.collection<CustomerEntity>().filter().serverIdEqualTo(customerId).findFirst();
    if (byServerId != null) return byServerId;

    // Try by phone
    final byPhone =
        await db.collection<CustomerEntity>().filter().phoneEqualTo(customerId).findFirst();
    return byPhone;
  }
  ...
  Future<CustomerEntity?> getCustomerByPhone(String phone) async {
    final db = await IsarDatabase.getInstance();
    return await db.collection<CustomerEntity>().filter().phoneEqualTo(phone).findFirst();
  }
  ...
  Future<CustomerEntity> registerCustomer({
    ...
    await db.writeTxn(() async {
      // Save customer
      await db.collection<CustomerEntity>().put(customer);
  ...
  Future<OtpVerification> sendTelegramOtp(String phone, String telegramId) async {
    ...
    await db.writeTxn(() async {
      ...
      final oldCodes = await db.collection<OtpVerification>()
          .filter()
          .phoneEqualTo(phone)
          .and()
      ...
      for (final old in oldCodes) {
        await db.collection<OtpVerification>().delete(old.id);
      }

      // Save new code
      await db.collection<OtpVerification>().put(verification);
      ...
      final queueItem = SyncQueueEntity()
        ...
        ..priority = 1;
      await db.collection<SyncQueueEntity>().put(queueItem);
    });
  ...
  Future<bool> verifyOtp(String phone, String code) async {
    ...
    final verification = await db.collection<OtpVerification>()
        .filter()
        .phoneEqualTo(phone)
        .and()
        .isVerifiedEqualTo(false)
  ...
  Future<CustomerStats> getCustomerStats(String salesRepId) async {
    ...
    final total = await db.collection<CustomerEntity>()
        .filter()
        .registeredByEqualTo(salesRepId)
        .count();

    final verified = await db.collection<CustomerEntity>()
        .filter()
        .registeredByEqualTo(salesRepId)
        .and()
    ...
    final withSubscriptions = await db.collection<CustomerEntity>()
        .filter()
        .registeredByEqualTo(salesRepId)
        .and()
    ...
    final todayCount = await db.collection<CustomerEntity>()
        .filter()
        .registeredByEqualTo(salesRepId)
        .and()
  ...
  Stream<List<CustomerEntity>> watchMyCustomers(String salesRepId) async* {
    ...
    yield* db.collection<CustomerEntity>()
        .filter()
        .registeredByEqualTo(salesRepId)
        .sortByCreatedAtDesc()
  ...
  Future<void> _queueCustomerUpdate(
    ...
    final queueItem = SyncQueueEntity()
      ..entityType = 'customer'
      ..entityId = customer.serverId ?? customer.id.toString()
      ..operation = operation
      ..payload = customer.toJson()
      ..status = 'pending'
      ..createdAt = DateTime.now()
