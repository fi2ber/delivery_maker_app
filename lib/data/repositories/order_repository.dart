...
class OrderRepository {
  ...
  Future<List<OrderEntity>> getDriverOrders(String driverId,
    ...
    var query = db.collection<OrderEntity>().filter().driverIdEqualTo(driverId);
  ...
  Future<OrderEntity?> getOrderById(String orderId) async {
    ...
    var order = await db.collection<OrderEntity>().filter().serverIdEqualTo(orderId).findFirst();

    // Fall back to local ID
    order ??= await db.collection<OrderEntity>().filter().orderCodeEqualTo(orderId).findFirst();
  ...
  Future<void> markAsPickedUp(String orderId) async {
    ...
    await db.writeTxn(() async {
      ...
      await db.collection<OrderEntity>().put(order);
  ...
  Future<void> saveOrdersFromServer(List<OrderEntity> orders) async {
    ...
    await db.writeTxn(() async {
      for (final order in orders) {
        ...
        final existing = await db.collection<OrderEntity>()
            .filter()
  ...
  Future<int> getPendingCount(String driverId) async {
    final db = await IsarDatabase.getInstance();
    return await db.collection<OrderEntity>()
        .filter()
  ...
  Future<OrderStats> getTodayStats(String driverId) async {
    ...
    final orders = await db.collection<OrderEntity>()
        .filter()
        .driverIdEqualTo(driverId)
        .and()
  ...
  Stream<List<OrderEntity>> watchOrders(String driverId) async* {
    ...
    yield* db.collection<OrderEntity>()
        .filter()
        .driverIdEqualTo(driverId)
        .sortByDeliveryDate()
  ...
  Future<int> cleanupOldOrders() async {
    ...
    final oldOrders = await db.collection<OrderEntity>()
        .filter()
        .statusEqualTo('delivered')
        .and()
    ...
    await db.writeTxn(() async {
      for (final order in oldOrders) {
        await db.collection<OrderEntity>().delete(order.id);
      }
    });
  ...
  Future<OrderEntity?> _findOrder(Isar db, String orderId) async {
    ...
    if (int.tryParse(orderId) != null) {
      ...
      final byLocalId = await db.collection<OrderEntity>().get(localId);
      if (byLocalId != null) return byLocalId;
    }

    // Try by server ID
    final byServerId =
        await db.collection<OrderEntity>().filter().serverIdEqualTo(orderId).findFirst();
    if (byServerId != null) return byServerId;

    // Try by order code
    final byCode =
        await db.collection<OrderEntity>().filter().orderCodeEqualTo(orderId).findFirst();
    return byCode;
  }
  ...
  Future<void> _queueOrderUpdate(
    ...
    final queueItem = SyncQueueEntity()
      ..entityType = 'order'
      ..entityId = order.serverId ?? order.id.toString()
      ..operation = operation
